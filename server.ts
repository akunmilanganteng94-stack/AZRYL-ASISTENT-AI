...
function getCandidateModels(configuredModel: string | undefined, baseUrl: string): string[] {
...
function isIdentityQuestion(rawMessage: string): boolean {
...
// GET / - Status API
// Jika diminta via curl, format=json, atau Accept: application/json, kembalikan JSON status
...
// GET /api/config-status - Status konfigurasi API tanpa membocorkan API key
...
app.post("/api/chat", async (req: Request, res: Response) => {
  try {
    ...
    // Jika pertanyaan jelas menanyakan identitas AI/developer, kembalikan teks persis
    ...
    if (!apiKey || apiKey.trim() === "" || apiKey === "ISI_API_KEY_SENDIRI") {
      return res.status(400).json({
        success: false,
        error:
          `AI_API_KEY belum dikonfigurasi di file .env. Silakan masukkan API key Anda di .env untuk mulai mengobrol dengan ${AI_IDENTITY.assistant}.`,
        sessionId
      });
    }

    // Siapkan pesan percakapan (System + riwayat percakapan terakhir + pesan saat ini)
    ...
    // Dapatkan kandidat model yang sesuai untuk dicoba (dengan auto-fallback jika 404 Model Not Found)
    ...
    for (const modelToTry of candidateModels) {
      try {
        ...
        // Jika status 404 (Model not found atau model no longer available) atau 503 (temporary high demand), coba model fallback berikutnya
        ...
        if (isNotFound || isUnavailable) {
          ...
          // Error otentikasi (401), invalid request (400), dll: hentikan loop
...
// POST /api/chat/clear - Bersihkan riwayat chat session
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`=========================================`);
    console.log(`🚀 AZRYL AI Server running on port ${PORT}`);
